...
├─────────────────────────────────────────────────────────────────────────┤
│  第3层：Skills / Functions / Tools                                  │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Skills（对应 Palantir Action Type）：原子事务 + 副作用         │   │
│  │ Functions（对应 Palantir Function）：派生计算 / AI 集成        │   │
│  │ Tools：基础原子操作（基础设施层）                              │   │
│  └─────────────────────────────────────────────────────────────┘   │
├─────────────────────────────────────────────────────────────────────────┤
...
- Entity 代表语义上有意义的概念（`Product`、`Order`、`Customer`），而非数据库表名
- Relation 代表真实关系（"商品 → 供应商"），而非外键
...
- 核心 Concept 保持稳定
- 通过 Interface 扩展抽象层
- 新业务需求通过新 Concept / Skill 实现，而非修改既有定义

**原则 4：组合优于深层继承（Composition over Inheritance）**
> 用 Interface 多继承替代深层类层次。

- 避免宽而稀疏的 Concept（包含大量可选字段）
- 用 Interface 对共享特征建模
- Concept 可以实现多个 Interface，支持多重多态

### Palantir 术语对照

| Palantir Foundry | 本文对应 | 说明 |
|----------------|---------|------|
| **Object Type** | **Concept** | 业务实体类型定义 |
| **Object** | **Entity** | 单个实体实例 |
| **Object Set** | **Entity Set** | 一组对象的集合 |
| **Link Type** | **Relation** | 关系类型定义 |
| **Interface** | **Interface** | 抽象接口，支持多态继承 |
| **Action Type** | **Skill（业务操作类）** | 原子事务 + 副作用 |
| **Function** | **Skill（推理分析类）** | 服务器端业务逻辑 |
...
### Concept（对应 Palantir Object Type）
...
Interface 描述 Concept 的**形状（shape）**和**能力**，提供多态性：
...
| | Concept（Object Type） | Interface |
|--|--------------------|-----------|
| 性质 | 具体（concrete） | 抽象（abstract） |
| 实例化 | 可以直接实例化 | 必须通过实现的 Concept 实例化 |
| 后端数据 | 有 backing datasource | 无 backing datasource |

### Property（属性）

Property 是 Concept 的特征，分为两种：

- **Local Property**：直接在 Concept 上定义
- **Shared Property**：跨多个 Concept 共享的标准化属性
...
**类型支持：** `string`、`int`、`float`、`bool`、`datetime`、`ref`（引用其他 Entity）

### Relation（对应 Palantir Link Type）

Relation 定义两个 Concept 之间的关系，Relation Instance 是该关系的单个实例。

**Cardinality（基数）：**

| 类型 | 说明 | 示例 |
|------|------|------|
| `one-to-one` | 一个 Entity 对应一个 Entity | 员工 → 工位 |
| `one-to-many` | 一个 Entity 对应多个 Entity | 供应商 → 商品 |
...
| 反模式 | 描述 | 正确做法 |
|--------|------|---------|
| **Kitchen Sink** | 一个 Concept 包含大量无关字段，镜像源系统 | 按领域实体拆分 Concept |
| **Golden Hammer** | 用 Pipeline/Function 处理本该用 Skill 的人工决策 | Skill 用于人类决策，Function 用于自动化 |
| **系统镜像** | Concept 等于源系统表 | 按业务语义建模 |
| **孤岛团队** | 单团队设计 Ontology | 多团队协作，防止重复 |
| **无文档** | 不记录 Concept/Property 业务含义 | 在 Ontology Manager 中完整记录元数据 |
...
## 第3层：Tools、Skills 和 Functions（对齐 Palantir）

### 对齐说明

| Palantir Foundry | 本文对应 | 说明 |
|----------------|---------|------|
| **Action Type** | **Skill（业务操作类）** | 原子事务，改变数据的操作单位 |
| **Function** | **Skill（推理分析类）** | 服务器端业务逻辑 |
| **Tools（基础设施）** | **Tools** | 基础设施类 Tool 不变 |
...
| Tool | 描述 |
|------|------|
| `ontology_read` | 读取 Ontology Schema（Concept、Property、Relation） |
| `ontology_write` | 创建/更新 Ontology Schema |
| `entity_search` | 查询 Entity（商品、客户等） |
| `entity_write` | 写入/更新 Entity |
| `entity_set_query` | 查询 Entity Set（对象集） |
| `external_api_call` | 调用外部零售系统 API（ERP、WMS、POS） |

### Skills（对齐 Palantir Action Type）

Skill 是 Ontology 中**改变数据的交易单位**，对应 Palantir 的 Action Type。

**Skill 的特点：**
- **原子性**：一笔交易，失败则回滚
- **可组合**：一次 Skill 可修改多个 Entity 的多个 Property
- **副作用（Side Effects）**：可发送通知、触发 Pipeline 等
- **授权控制**：通过 submission criteria 限制谁能执行

```
Skill Type: Assign Employee Role（分配员工角色）
    ├── 参数定义：用户输入新角色（表单）
    ├── 业务逻辑：修改 Employee.role property
    ├── 自动行为：在 Employee ↔ Manager 之间创建 Relation
...
| 聚合计算 | Entity Set 聚合统计 |
| 复杂查询 | 跨多个 Entity 的复杂过滤 |
...
| Skill | 描述 |
|-------|------|
| `product_query` | 按品类、价格区间、销量查询商品 |
| `customer_query` | 查询客户信息 |
| `order_summary` | 生成订单统计 |
| `inventory_alert` | 库存低于阈值时预警 |
| `calculate_reorder_point` | 计算补货点（Function-backed） |

**MVP 简化：**
- 无 Skill/Function 注册系统，v1 版为硬编码
- 无 Skill 版本管理或热加载
...
```
用户 A：可查看自己创建的 Relation
用户 B：可查看分配给自己的 Relation
管理员：可查看所有 Relation
```
